...
internal interface IMMDeviceCollectionCOM
{
    [PreserveSig]
    int GetCount(out uint devices);

    [PreserveSig]
    int Item(uint deviceIdx, [MarshalAs(UnmanagedType.Interface)] out IMMDeviceCOM device);
}