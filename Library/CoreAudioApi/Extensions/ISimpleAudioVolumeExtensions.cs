...
{
    public static int SetMasterVolume(this ISimpleAudioVolumeCOM simpleAudioVolume, float level,
        Guid? eventContext = null)
    {
        unsafe
        {
            return eventContext is { } ctx
                ? simpleAudioVolume.SetMasterVolume(level, &ctx)
                : simpleAudioVolume.SetMasterVolume(level, null);
        }
    }

    public static int SetMute(this ISimpleAudioVolumeCOM simpleAudioVolume, bool value,
        Guid? eventContext = null)
    {
        int mute = value ? 1 : 0;
        unsafe
    ...
    public static int GetMute(this ISimpleAudioVolumeCOM simpleAudioVolume, out bool mute)
    {
