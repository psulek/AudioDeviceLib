...
namespace AudioDeviceLib.Test
{
    internal static class Program
    {
        ...
        private static int CmdList(AudioController audio, Options o)
        {
            ...
            if (o.Has("recording")) devices = audio.GetRecordingDevices();
            else if (o.Has("playback")) devices = audio.GetPlaybackDevices();
            else devices = audio.GetDevices();
        ...
        {
            DefaultRole role;
            if (!TryGetRole(o, out role)) return 1;
        ...
        private static AudioDevice ResolveSelector(AudioController audio, Options o)
        {
            ...
            if (id != null)
            {
                ...
                if (match == null) Console.Error.WriteLine("No device found with ID: " + id);
            }
            else if (indexStr != null)
            {
                ...
                if (match == null) Console.Error.WriteLine("No device found with index: " + idx);
            }
            else
            {
        ...
        private static Options ParseOptions(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if (a.StartsWith("/") || a.StartsWith("-")) a = a.Substring(1);
                else continue; // positional args are ignored; selectors are explicit
