...
public class DeviceTests
{
    ...
    public void Properties_TryGetValue_PresentKey_ReturnsTrue()
    {
        ...
        bool found = device.Properties.TryGetValue(PKEY.PKEY_Device_FriendlyName, out var value);
    ...
    public void PropertyStoreProperty_Value_SurvivesTheVariantBeingReleased()
    {
        ...
        PropertyStoreProperty property = device.Properties[PKEY.PKEY_Device_FriendlyName];
    ...
    public void Properties_AudioEndpointKeys_AddressDistinctProperties()
    {
        ...
        PropertyKey[] keys =
        {
            ...
            PKEY.PKEY_AudioEndpoint_GUID,
            PKEY.PKEY_AudioEndpoint_JackSubType,
        };
        ...
        foreach (PropertyKey key in keys)
        {
            ...
            Assert.That(property.Key.pid, Is.EqualTo(key.pid),
                $"lookup for {key.Name} returned pid {property.Key.pid}; the key is not selecting on PID");
            resolved.Add(property.Key);
        }

        Assert.That(resolved, Is.Not.Empty, "no AudioEndpoint properties present on this endpoint");
        Assert.That(resolved.ConvertAll(k => k.pid), Is.Unique);
    }
    ...
    public void Properties_DeviceAndInterfaceFriendlyName_AreReadSeparately()
    {
        ...
        bool hasDeviceName = device.Properties.TryGetValue(PKEY.PKEY_Device_FriendlyName, out var deviceName);

        Assert.That(hasDeviceName, Is.True);
        Assert.That(deviceName.Value, Is.EqualTo(device.Name));

        if (device.Properties.TryGetValue(PKEY.PKEY_DeviceInterface_FriendlyName, out var interfaceName))
        {
            Assert.That(interfaceName.Value, Is.Not.Null);
            Assert.That(device.Properties[PKEY.PKEY_DeviceInterface_FriendlyName].Key.fmtid,
                Is.Not.EqualTo(device.Properties[PKEY.PKEY_Device_FriendlyName].Key.fmtid),
