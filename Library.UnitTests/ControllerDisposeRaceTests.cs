...
public class ControllerDisposeRaceTests
{
    ...
    public void Dispose_WithAStuckOperation_GivesUpRatherThanHanging()
    {
        ...
        // Stands in for a COM call that never returns: takes a lease and holds it.
        using var releaseLease = new ManualResetEventSlim(false);
        using var leaseTaken = new ManualResetEventSlim(false);
        var holder = new Thread(() =>
        {
            using (audio.AcquireEnumeratorForTest())
            {
                leaseTaken.Set();
                releaseLease.Wait();
            }
        ...
        try
        {
            Assert.That(leaseTaken.Wait(TimeSpan.FromSeconds(10)), Is.True, "lease was never taken");
            disposer.Start();

            // The drain timeout is five seconds; well under the wait here. The point is that Dispose
            // returns at all rather than blocking on the lease that is still held.
            Assert.That(disposed.Wait(TimeSpan.FromSeconds(30)), Is.True,
                "Dispose blocked on an operation that never completed");
        }
        finally
        {
            // Unconditional: a failed assertion must not leave these threads parked forever.
            releaseLease.Set();
