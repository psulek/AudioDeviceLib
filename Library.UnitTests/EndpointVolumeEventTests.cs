/*
  ...
  points that take a `ref Guid pguidEventContext`. One AudioController subscribes to the endpoint's
...
public class EndpointVolumeEventTests
{
    ...
    private AudioController _listener;
    private AudioController _driver;
    private AudioDevice _listenerDevice;
    private AudioDevice _driverDevice;
    private VolumeNotificationRecorder _recorder;

    private float _originalMasterScalar;
    private bool _originalMute;
    private float[] _originalChannelScalars;

    // The raw COM interface the extension methods extend, reached through the driver controller.
    private IAudioEndpointVolume Driver => _driverDevice.Volume.Interface;
    ...
    public void SetUp()
    {
        ...
        _driverDevice = _driver.GetDeviceById(_listenerDevice.Id);
        ...
        _listenerDevice.Volume.OnVolumeNotification += _recorder.Record;
    }

    [TearDown]
    public void TearDown()
    {
        if (_listenerDevice != null && _recorder != null)
        {
            _listenerDevice.Volume.OnVolumeNotification -= _recorder.Record;
        }
    ...
    public void SetChannelVolumeLevelScalar_NotifiesWithEventContext()
    {
        ...
        Assert.That(data.ChannelVolume, Has.Length.EqualTo(data.Channels));
    ...
    // would have reported for a null pguidEventContext anyway.
    [Test]
